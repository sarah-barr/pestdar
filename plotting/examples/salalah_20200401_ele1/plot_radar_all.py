...
def main():
    ...
    if 'Delhi' in dpath:
        for fname in fnames:
            ...
            plt_params = delhi_plot_setup(fpath, ele)
            sname = os.path.splitext(fname)[0] + '_{}km'.format(r) + '_ele{:.0f}'.format(plt_params[9]) + '.png'
            sdir = os.path.join(spath, 'range_{}km'.format(r), 'elevation_{:.0f}'.format(plt_params[9]), date)
            sfile = os.path.join(sdir, sname)
            if not os.path.exists(sdir):
                os.makedirs(sdir)
                radar_plotting(plt_params[0], ele, r, plt_params, sfile)
            else:
        ...
        print('Salalah plotting')
        for fname in fnames:
            fpath = os.path.join(dpath, fname)
            plt_params = salalah_plot_setup(fpath, ele)
            if len(plt_params[1]) != 8:
              err_count += 1
              print('Incorrect fields in {}. Expected 8 but found {}'.format(fname, len(plt_params[1])), flush = True)
              continue
            sname = os.path.splitext(fname)[0] + '_{}km'.format(r) + '_ele{:.0f}'.format(plt_params[9]) + '.png'
            sdir = os.path.join(spath, 'range_{}km'.format(r), 'elevation_{:.0f}'.format(plt_params[9]), date)
            sfile = os.path.join(sdir, sname)
...
def delhi_plot_setup(fpath, ele):
    ...
    return radar, fields, plt_names, plt_cols, vmin, vmax, units, plt_order, instrument_name, elevation
...
def radar_plotting(radar, ele, r, plt_params, sfile):
    display = pyart.graph.RadarMapDisplay(radar)

    ## Plotting Options
    ele = ele #Elevation
    r = r
    radar, fields, plt_names, plt_cols, vmin, vmax, units, plt_order, instrument_name, elevation = plt_params
